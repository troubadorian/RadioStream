...
import com.troubadorian.streamradio.controller.TriggerActivity;
...
//			debug( "handleTransaction " + code );
		...
		case kQuit: 
		    //stop IHRServiceCofiguration runFetchStartup (endless try)
		    // following line added by lance on 27 sep, 2010 for endless loop fix
            mConfiguration.serviceStopped = true;
		    stopSelf(); 
		    break;
	...
	public Notification createNotification() {
		if ( null == mNotification ) {
			...
			//mNotification.contentIntent = PendingIntent.getActivity( this , 0 , new Intent( this , Streamradio.class ).addFlags( Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_FROM_BACKGROUND )  , PendingIntent.FLAG_UPDATE_CURRENT );
...
//			mNotification.contentView = new RemoteViews( "com.clearchannel.iheartradio.controller" , R.layout.notification );
			...
			mNotification.tickerText = "Streamradio";	//	metadata
			mNotification.vibrate = null;
			mNotification.when = System.currentTimeMillis();
			mNotification.setLatestEventInfo( this , "Streamradio" , "" , mNotification.contentIntent );
		}
	...
	public void noticeStreaming( boolean inStreaming ) {
		if ( inStreaming ) {
			if ( null == mWifi ) {
				try {
					mWifi = ((WifiManager)this.getSystemService( WIFI_SERVICE )).createWifiLock( WifiManager.WIFI_MODE_FULL , "Streamradio" );
				} catch ( Exception e ) {}
			}
